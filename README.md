...
```
https://login.salesforce.com/packaging/installPackage.apexp?p0=04tId000000cElsIAE
```

**Sandbox:**

```
https://test.salesforce.com/packaging/installPackage.apexp?p0=04tId000000cElsIAE
```
...
#### チャットモード（Einstein Copilot サイドパネル）

ユーザーが Einstein Copilot に自由に依頼します。
...
#### ヘッドレスモード（クイックアクションボタン）

レコードページ（またはグローバルアクションバー）の **「顧客リサーチ」** ボタンをクリックするだけで、確認なしでリサーチを自動実行・保存します。

- **Account / Opportunity / Case のレコードページから起動:** `recordId` から取引先名を自動解決してリサーチを実行
- **グローバルアクションバーから起動:** 取引先選択画面を表示
- **決算サマリーの自動追加:** 直近2週間以内に決算があり、まだ決算サマリー Topic が未作成の場合、顧客リサーチと同時に決算サマリーも自動生成

```
ボタンクリック
  ↓
Screen Flow（Customer_Research_Action）
  ├── recordId あり → Account / Opportunity / Case から取引先名を解決
  └── recordId なし → 取引先選択画面
  ↓
CheckRecentEarnings で直近決算チェック
  ↓
AgentforceHeadlessResearch で顧客リサーチ実行（バックグラウンド）
  ↓（決算あり・未作成の場合）
AgentforceHeadlessResearch で決算サマリーも実行
  ↓
「✅ リサーチを開始しました」完了画面
```
...
```
agent_router
  ├── web_research         — Webリサーチ・ニュース（決算サジェスト機能付き）
  ├── competitive_analysis — 競合分析
  ├── earnings_summary     — 決算サマリー
  ├── headless_research    — ヘッドレス自動実行（ボタン起動時）
  ├── save_topic           — 保存確認・実行（チャットモード共通）
  ├── off_topic            — スコープ外の応答
...
デプロイ後、Quick Action をレコードページのアクションバーに手動で追加します。

**レコードページへの配置（Account / Opportunity / Case）:**
1. Setup → Object Manager → 対象オブジェクト → Lightning Record Pages
2. 編集するページを開く → Lightning App Builder
3. アクションバーコンポーネントの **編集** → Actions 一覧から `顧客リサーチ` を追加
4. 保存 → Activate

**グローバルアクションバーへの配置:**
1. Setup → **Global Actions** → **Publisher Layouts**
2. `Global Layout` を編集
3. Quick Actions 一覧から `顧客リサーチ` をドラッグして追加
4. 保存
...
| CheckRecentEarnings             | **Invocable Action** — 直近2週間の決算有無と既存 Topic の確認 |
| AgentforceHeadlessResearch      | **Invocable Action** — エージェントをヘッドレスで起動（Screen Flow から呼び出し） |
...
**SubscriberPackageVersionId**: `04tId000000cElsIAE`  
